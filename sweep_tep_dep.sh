...
# Constraint: TP × EP × DP = 8 GPUs
#
# Configs (TEP → DEP):
#   TP=8  EP=1  DP=1   Pure TEP   — attention+experts sharded 8-way, lowest latency
#   TP=4  EP=2  DP=1   Hybrid     — 4-way attention shard, 2 EP groups
#   TP=2  EP=4  DP=1   Hybrid     — 2-way attention shard, 4 EP groups
#   TP=1  EP=8  DP=1   Pure DEP   — full attention per GPU, 8 EP groups
#
# Memory per GPU (FP8, ~37B non-expert, ~634B expert):
#   TP=8 EP=1: ~84 GB   →  ~108 GB free for KV cache
#   TP=4 EP=2: ~89 GB   →  ~103 GB free
#   TP=2 EP=4: ~98 GB   →  ~94 GB free
#   TP=1 EP=8: ~116 GB  →  ~76 GB free
#
...
# TP,EP pairs — override with e.g. CONFIGS="8,1 4,2"
if [[ -n "${CONFIGS:-}" ]]; then
...
echo "tp,ep,dp,concurrency,isl,osl,throughput_req_per_s,throughput_tok_per_s,mean_ttft_ms,median_ttft_ms,p99_ttft_ms,mean_tpot_ms,median_tpot_ms,p99_tpot_ms,mean_e2el_ms,median_e2el_ms,p99_e2el_ms" \
    > "$SUMMARY_CSV"
...
extract_metrics() {
    local json_file=$1 tp=$2 ep=$3 dp=$4 conc=$5
    python3 -c "
import json, sys
with open('$json_file') as f:
    d = json.load(f)
row = [
    $tp, $ep, $dp, $conc, $ISL, $OSL,
...
for config in "${CONFIG_PAIRS[@]}"; do
    IFS=',' read -r TP EP <<< "$config"
    DP=$(( NUM_GPUS / (TP * EP) ))

    for CONC in "${CONCURRENCIES[@]}"; do
        NUM_PROMPTS=$(( CONC * NUM_PROMPTS_MULT ))
        TAG="tp${TP}_ep${EP}_dp${DP}_conc${CONC}_isl${ISL}_osl${OSL}"

        log "============================================================"
        log "Run: TP=$TP  EP=$EP  DP=$DP  conc=$CONC"
        log "============================================================"
        ...
        extract_metrics "${RESULT_DIR}/${TAG}.json" "$TP" "$EP" "$DP" "$CONC"

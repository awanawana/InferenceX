...
# ==================== Single-Node Path ====================
# For non-disaggregated frameworks (sglang, trt, vllm), use the single-node
# Slurm path similar to B200. Enroot + squashfs container on the B300 cluster.
if [[ $FRAMEWORK == "sglang" || $FRAMEWORK == "trt" || $FRAMEWORK == "vllm" ]]; then
    export HF_HUB_CACHE_MOUNT="/scratch/hf_hub_cache/"
    export PORT=8888
    ...
    bash benchmarks/single_node/${MODEL_CODE}_${PRECISION}_b300${FRAMEWORK_SUFFIX}${SPEC_SUFFIX}.sh

    scancel $JOB_ID
    exit $?
fi

# ==================== Multi-Node Path (Dynamo) ====================
# Validate framework for multi-node
if [[ $FRAMEWORK != "dynamo-sglang" && $FRAMEWORK != "dynamo-trt" ]]; then
    echo "Unsupported framework: $FRAMEWORK. Supported frameworks are: sglang, trt, vllm, dynamo-trt, dynamo-sglang"
    exit 1
fi
